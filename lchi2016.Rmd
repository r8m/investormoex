---
title: "Trades Visualizer version 0.7   "
runtime: shiny
output:
...
statLink<-"https://github.com/r8m/moexinvestor/raw/master/resDT.RData"
...
```{r side}
 renderUI({selectInput("exch", "Рынок:",
             markets,selected=markets[marketSelected()[[1]][1]])})

 renderUI({
   userChoices<-c("Выберите"="", "->Свои сделки<-"=1, f)
   selectInput("nick", "Участник:",userChoices,selected=userChoices[userSelected()[[1]][1]])
   })
...
userStat<-reactive({
  ...
  if ( userId()=="") return (NULL)
  else{  res<-isolate(makePortfolio(yearId, 
                     userId(),
                     marketId(), 
                     nick=userSelected()[[1]][1], 
                     amount=100000, 
...
output$tbl<-DT::renderDataTable(resDT,
                    filter = 'top',
                    selection = 'single',
                options = list(
                  pageLength = 10,
